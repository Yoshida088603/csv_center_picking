...
CSVに記載された座標周辺の点群（LAZ/LAS）を抽出するツール。サーバー不要のブラウザ版をメインに検証しています。
...
サーバー不要でブラウザ上でLAZ/LASを処理できます。1GB以上のファイルも対応可能です。
...
laz-perf WASMを統合し、GitHub Pagesのみで動作する完全サーバーレスなLAZ処理アプリケーションを実現しています。
